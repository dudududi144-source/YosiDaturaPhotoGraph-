...
export default function Lightbox() {
  ...
  return (
    <div className={`lightbox ${isOpen ? 'active' : ''}`} role="dialog" aria-modal="true" aria-label="Image viewer" aria-hidden={!isOpen} onClick={close}>
      ...
      <button onClick={(e) => { e.stopPropagation(); close(); }}
        className="absolute top-4 md:top-6 right-6 font-mono text-xs text-white/60 hover:text-white transition-colors duration-300 z-10 p-2" aria-label="Close viewer">
        ✕ CLOSE
      </button>
      {frame && (
        <>
          <img key={frame.id} src={asset(frame.img)} alt={frame.caption} onError={handleError} onClick={(e) => e.stopPropagation()} />
          <div className="font-mono text-xs tracking-[0.25em] text-white/60 text-center px-6">{frame.caption.toUpperCase()}</div>
